 ...
 *
 * @param value A JSON Web Token (JWT) generated by the client backend
 * which is bound to a key managed by a Client Instance which can then
 * be used by the instance for client authentication
 */
@JvmInline
value class ClientAttestationJWT private constructor(val value: SignedJWT) {
    companion object {
...
inline fun <reified T : Any> ClientAttestationJWT.decodeClaimsSet(): T = value.jwtClaimsSet.decodeAs<T>().getOrThrow()
