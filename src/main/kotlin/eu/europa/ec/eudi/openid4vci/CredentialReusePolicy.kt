...
enum class ReuseMethod {
    ...
    companion object {
        fun fromJsonValue(value: String): ReuseMethod? = entries.firstOrNull { it.jsonValue == value }
    }
}
 ...
 *
 * @param details the reuse methods for this option; must contain either [ReuseMethod.ONCE_ONLY] or [ReuseMethod.LIMITED_TIME] (but not both)
 ...
 */
data class ReusePolicyOption(
    val details: List<ReuseMethod>,
    ...
    init {
        val hasOnceOnly = ReuseMethod.ONCE_ONLY in details
        val hasLimitedTime = ReuseMethod.LIMITED_TIME in details
        val hasRotatingBatch = ReuseMethod.ROTATING_BATCH in details
        val hasPerRelyingParty = ReuseMethod.PER_RELYING_PARTY in details
     ...
     */
    fun isSupported(supportedReuseMethods: Set<ReuseMethod>): Boolean =
 ...
 *
 * @param id identifies the reuse policy
 * @param options extra details about the specific reuse policy
 */
data class CredentialReusePolicy(
    val id: String,
    val options: List<ReusePolicyOption>? = null,
) : Serializable {

    init {
        require(id.isNotBlank()) { "Reuse policy id must not be blank" }
        if (id == ARF_ANNEX_II_POLICY_ID) {
            requireNotNull(options) { "options is mandatory for arf_annex_ii policy" }
     ...
     */
    fun effectiveBatchSize(supportedReuseMethods: Set<ReuseMethod>): Int? {
        if (options.isNullOrEmpty()) return null
        return options
            .filter { it.isSupported(supportedReuseMethods) }
            .firstNotNullOfOrNull { it.batchSize }
    }

    companion object {
        const val ARF_ANNEX_II_POLICY_ID = "arf_annex_ii"

        private fun validateNoOverlappingDetails(options: List<ReusePolicyOption>) {
            if (options.size <= 1) return
            val normalizedDetailsCombinations = options.map { option ->
                option.details
                    .sortedBy(ReuseMethod::name)
